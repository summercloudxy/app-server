package com.zgiot.app.server.module.sfmonitor.constants;

public class SFSysMonitorConstant {
    public static final String SFSYS_MONITOR_THING_TAG_GROUP_CODE  = "SFSYSMON";//系统监控thing_tag_group CODE值
    public static final short TERM_ONE=1;//一期
    public static final short TERM_TWO=2;//二期
    public static final String THING_CODE_QUIT_SYS_RAW="Quit_SYS_RAW";//原煤数质量thingCode
    public static final short SHOW_TYPE_2=2;//设备显示类型2代表空白
}
                                                  