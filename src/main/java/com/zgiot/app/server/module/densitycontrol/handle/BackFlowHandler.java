...
@Component
public class BackFlowHandler implements DensityControlHandler {
    @Autowired
    ...
    private void init() {
        notifyIntervalMap =
                densityControlMapper.getNotifyIntervalByNotifyType(DensityControlManager.NOTIFY_TYPE_BACK_FLOW);
    }

    @Override
    public void dispose(MonitoringParam monitoringParam) {
        // logger.debug(
        // "设备{}应该开启回流弹窗通知，关闭其他弹窗通知，当前的密度智能控制各项参数值为：当前处于停车前状态{}，当前比例阀开度为{}，设定高开度为{}，设定低开度为{}，停车前设定低开度为{}，当前密度{}，设定密度{}，密度波动值{}，当前液位{}，设定高液位{}，设定低液位{}，停车前设定高液位{}",
        ...
        logger.debug("设备{}应该开启回流弹窗通知，关闭其他弹窗通知", monitoringParam.getThingCode());
        int interval = notifyIntervalMap.get(monitoringParam.getThingCode()).getInterval();
        monitoringParam.setCurrentStage(DensityControlManager.NOTIFY_TYPE_BACK_FLOW);
...
//                }
//            };
            QuartzManager.addJobWithMinutes(DensityControlManager.NOTIFY_TYPE_BACK_FLOW, monitoringParam.getThingCode(),
                    DensityControlManager.NOTIFY_TYPE_BACK_FLOW, monitoringParam.getThingCode(), NotifyJob.class,
                    interval, null);
            logger.debug("设备{}目前不存在回流弹窗通知，开启该推送", monitoringParam.getThingCode());
        }
    }

    public void cancel(String thingCode) {
        if (QuartzManager.checkExists(DensityControlManager.NOTIFY_TYPE_BACK_FLOW, thingCode)) {
            QuartzManager.removeJob(DensityControlManager.NOTIFY_TYPE_BACK_FLOW, thingCode,
                    DensityControlManager.NOTIFY_TYPE_BACK_FLOW, thingCode);
            logger.debug("设备{}目前存在回流弹窗通知，关闭该推送", thingCode);
        }
    }
}
