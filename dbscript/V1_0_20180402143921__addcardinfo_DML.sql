...
CREATE TABLE `tb_production_inspect` (
  ...
  `param_-1.45` float(10,4) DEFAULT NULL COMMENT '灰分(%), 2位小数 ',
  `param_1.45-1.8` float(10,4) DEFAULT NULL COMMENT '水份(%), 1位小数 ',
  `param_+1.8` float(10,4) DEFAULT NULL COMMENT '硫份(%), 2位小数 ',
  `param_+1.45` float(10,4) DEFAULT NULL COMMENT '发热量, 整数(kal/kg) ',
  `param_-1.8` float(10,4) DEFAULT NULL COMMENT '系统，1：一期， 2：二期',
  `param_+50mm` float(10,4) DEFAULT NULL,
  `param_-50mm` float(10,4) DEFAULT NULL,
